 ...
 * 由于缓存容量有限，我们需要剔除一些元素，给新元素腾出来空间，那么剔除哪个元素就就需要使用LRU。同样的，另外一个问题是，使用缓存我们要的就是快速查找，那么推荐
 * 的结构就是哈希表，所以这里就引申出了一种新的数据结构，即哈希链表。
 *
 * 我们知道哈希表在逻辑上是无所谓排列顺序的，而链表是一种有固定顺序的线性结构，将二者结合即为哈希链表。
 * 哈希链表，首先是一个哈希表，我们知道哈希表是key-value的键值对；哈希链表中的value比较特殊，是一个node，这个node里存储了key、value、prev和next指针。
 ...
 * 中的元素时，找到它对应的节点，然后把它从哈希链表中删除，重新插入到最右侧，此时最右侧节点仍为最近访问过的节点，最左边的节点仍然是最最近最少使用的。如果空间不足时，
 * 直接删除最左边的节点即可。
 *
 */
...
function createLRUCache(limit) {
    ...
    function Node(key, value) {
        this.key = key; // 这个key是需要的，用来从node反向查找哈希表的key，在下面removeNode操作中会被返回，用于在put方法中删除哈希表中的一项
        this.value = value;
        this.pre = null;
        this.next = null;
    }
