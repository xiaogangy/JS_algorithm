...
function solution(A) {
    ...
    // 可以在构建C的时候直接就把C的数据填充的到B，就会省掉一个临时数组(C)的空间。
    // 那么有了C之后还需要D吗，其实是不需要的，我们沿着一个数组的方向填充B（下面的写法是倒着填充），变短的那一侧（C）是需要一开始构造
    // 一个临时数组的，另外一侧是长度逐渐变长，只需要保存上一次的元素，然后计算下一次的时候，再乘以一个新的A[i]元素。
    ...
    // 2. 声明一个变量保存D
    let temp = 1; // 初始化为1，因为填充B的时候是倒着填充的
