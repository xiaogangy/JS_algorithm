 ...
 * 如果我们想要O(1)的空间复杂度怎么办？那么哈希表肯定就不能考虑了。我们考虑一下这个题的特性，某个数字出现的次数超过了数组长度的一半，那么如果排序，数组的中位数一定就是这个待查找的数字。
 * 我们有成熟的时间复杂度为O(N)算法来得到数组中任意第K大的数字，这就要借助快排中partition函数的思路。我们知道partition函数是在数组中随机找一个元素作为pivot，然后把小于pivot的值放
 * 到数组的左边，大于pivot的放在右边，然后返回pivot这个元素应该处于的位置，也就是index值，这时候如果index刚刚好就是数组的正中间位置，那么我们自然就找到了这个数；如果index大于数组
 * 中位数的位置，那么说明待查找的值还在前半段，这个时候，再缩小end的值，对前半段进行partition划分；如果index小于中位数的位置呢？那么中位数肯定在数组后半部分，此时我们只需要再对数组
 * 后半部分进行partition就可以了。
 * 时间复杂度：O(N)，空间复杂度：O(1)
 *
 * solution3：找规律
 * solution1需要额外的空间，solution2需要改变数组，那么有没有一种方法，即不需要额外空间，又不改变数组，还能时间复杂度达到O(N)呢？不好意思，还真有！
 ...
 * 就把次数减去1；如果发现当前的次数已经是0，而新的数字和之前保存的数字还不一样，那么我们就把新的数字保存起来，次数置为1。持续这样进行，数组遍历完后，保存的数字就是待查找的数字。
 * 时间复杂度：O(N)，空间复杂度：O(1)
 */
