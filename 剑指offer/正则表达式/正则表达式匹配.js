 ...
 * 第二个字符，再看第一个字符。所以准确来说，步骤应该是2 --> 1。那么为*的情况下如何处理呢？这里又要分两种情况：
 * 1）前面的字符已经匹配了
 * 则有三种状态可以变化：
 * - 1. *和它前面的字符都算作用完了，正则进2，字符串进1
 * - 2. 字符串进1，正则不变，表示正则打算拿这个*匹配更多的字符
 * - 3. 字符串不动，正则进2，表示即使你匹配到我字符串中的这个字符了，但是我不想要这次的匹配，想要下次的。准确说应该是我这个正则只匹配了0次的情况
 * 因此上面三种情况，其实就对应着*匹配0次，1次，多次的情况。
 * 2）如果前面的字符没有匹配到呢？
 * 简单，只让正则进2，意思是拿后面的正则继续来匹配当前的字符
 *
 * 如果第二个字符不是*呢？
...
/**
 * 为了降低空间复杂度，用了两个索引来track匹配过程，向C++看齐
