 ...
 * 此时我们抽象出了这个问题的递归解决方案，但是递归出口怎么考虑呢？一般情况下，递归就是找最小规模的问题的解决方案，这道题里，我们可以发现如果前序遍历和中序
 * 遍历都是只有一个元素的话，那么自然就直接是一棵树了。这就是这道题的递归出口。
 */
...
function binaryTreeRecons(preOrder, inOrder) {
    ...
    let rootPos = 0;
    while (rootPos < inOrder.length && inOrder[rootPos] !== rootValue) {
        rootPos++;
    }
