 ...
 * 并每次向右或者向下移动一格，直到到达棋盘的右下角。给定一个棋盘及其上面的礼物，请计算你最多能拿到多少价值的礼物？
 ...
 * - 无后效性：当我求f(i, j)的时候，只关心f(i, j - 1)和f(i - 1, j)，而不关心他们的是怎么得来的了，因为它们怎么得来不影响我后面的计算
 * 接下来我们看看这道题的题解，定义f(i, j)为达到格子(i, j)时所能得到的礼物的最大值，我们知道到达格子(i, j)只能从左边(i-1, j)或者上边(i, j-1)达到，
 * 所以我们可以得到递推公式：f(i, j) = max(f(i-1, j), f(i, j-1)) + g(i, j); 其中g(i, j)表示坐标为(i, j)的格子里的礼物价值。
