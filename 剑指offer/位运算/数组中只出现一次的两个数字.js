 ...
 * 因为同一个进行异或运算，结果为0，而且0与任何数做异或运算，结果仍然是那个数字；所以异或运算最后的结果就是只出现了一次的那个数字。
 * 那么现在问题是有两个数字都缺失了，我们仍然可以沿用做异或运算的这个思路，对数组中的所有数字都做异或运算。异或运算的结果是两个出现了一次的数字的异或结果，因为这两个数字不相同，所以显然
 * 异或结果是有一个位置为1的，我们不妨找到这个位置，假设是二进制的第3位。然后我们可以根据2进制第三位数字是不是1将数组中所有的数字都分为两拨，这样的话每一侧都只有一个出现了一次的数字，而
 * 重复出现的数字仍然在一侧中，这时候我们就又可以用上面的的方法来求解了。
 *
 */
