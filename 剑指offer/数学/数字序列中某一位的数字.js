 ...
 * 请写一个函数，求任意第n位对应的数字。
 *
 * 思路：我们可以统计每种指定位数的数字总长度，然后判断要寻找的数字是否落在这个区间里。
 * 以一个实际的例子来说，比如我们要找第1001位，因为题目是从第0位开始计数的，所以实际上是找第1002位，所以我们就按照第1002位分析。
 * 我们知道1位数总共是0~9，共10个，远小于1002位，那么1002位肯定在后面。这时候我们知道要找从第一个2位数开始的，第1002-10 = 992位了。
...
function digitAtIndex(index, digit) {
    ...
    // 考虑余数为0的情况
    if (complement === 0) {
        ret = String(beginNumber + result - 1)[digit - 1];
    } else {
        ret = String(beginNumber + result)[complement - 1];
    }
