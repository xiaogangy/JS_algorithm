 ...
 *
 * solution2: 定义排序规则
 * 我们定义一种排序规则，两个数字，例如A和B，AB和BA都表示两个数字拼成的新数字。如果AB < BA，我们就说A小于B，反之，如果AB > BA，我们就说A大于B，这样的话就可以确定数组中二者的位置。
 * 经过这个排序规则对整个数组排序，我们可以得到一个排序后的序列，将这个序列转成整数，就是最小的数字。但是在进行AB和BA的大小比较时，如果直接将二者转成整数，有可能溢出，所以这里会出现
 * 大数比较的问题，所以我们把拼接成的AB和BA转成字符串再进行比较。
 * JS中字符串比较的规则是：根据第一个不同的字符的ASCII码值进行比较，其实就是字典序比较，这里AB和BA的长度一样，直接字典序比较非常可靠。
 * 具体的证明看剑指offer。
 *
 * solution3: 找规律
 * 拿题目中的例子来看，我们知道无论怎么排，最后组成的数字位数一定是一样的，所以这时候唯一能决定数值大小的就只剩下高位的元素了。要想让拼成的数值尽量小，那就让高位尽量小，所以
 ...
 * 元素判断位置了，321的第3位是1，而32没有第三位，如果把32放在321前面，显然第三位就更大，所以要把321放在32的前面。（其实这里就有点类似上面solution2比较的意思了）
